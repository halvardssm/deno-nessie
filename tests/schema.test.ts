...
const strings = [
	{
		...
		solution: "CREATE TABLE testTable (id bigserial, created_at timestamp (0) default now(), updated_at timestamp (0) default now()); DROP TRIGGER IF EXISTS set_timestamp on some_table; CREATE TRIGGER set_timestamp BEFORE UPDATE ON public.testTable FOR EACH ROW EXECUTE PROCEDURE trigger_set_timestamp();",
	}, {
		name: "Schema drop",
		string: (() => {
			const testSchema = new Schema()
		...
		string: Schema.hasTable('testTable'),
