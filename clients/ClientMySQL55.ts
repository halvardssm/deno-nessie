...
import type { AmountMigrateT, AmountRollbackT, DBDialects, QueryT } from "../types.ts";
...
export class ClientMySQL extends AbstractClient<MySQLClient> {
  ...
  async rollback(amount: AmountRollbackT) {
    ...
    await this._rollback(amount, allMigrations, this.query.bind(this));
  }
  ...
  async getAll() {
    ...
    const parsedMigrations: string[] = allMigrations?.[0].map((el: Record<string, string>) => el?.[COL_FILE_NAME]);
