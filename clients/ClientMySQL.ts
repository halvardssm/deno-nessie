...
export class ClientMySQL extends AbstractClient implements ClientI {
  ...
  private QUERY_MIGRATION_TABLE_EXISTS = `show tables like '${this.TABLE_MIGRATIONS}';`
  ...
  async prepare(): Promise<void> {
    ...
    const migrationTableExists = (await this.query(this.QUERY_MIGRATION_TABLE_EXISTS)).length > 0
